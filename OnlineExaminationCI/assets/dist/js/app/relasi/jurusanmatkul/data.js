...
$(document).ready(function() {
  ...
  table = $("#jurusanmatkul").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: base_url + "department-course/data",
      type: "POST"
    },
    columns: [
      {
    ...
    columnDefs: [
      {
        ...
        render: function(data) {
          ...
          $.each(list, function(i, val) {
            var item = `<span class="badge bg-green">${val}</span>`;
            badge.push(item);
          });
          return badge.join(" ");
        }
      },
      {
        ...
        render: function(data) {
          return `<div class="text-center">
            <a href="${base_url}department-course/edit/${data}" class="btn btn-warning btn-xs">
  ...
  $("#bulk").on("submit", function(e) {
    if ($(this).attr("action") == base_url + "department-course/delete") {
      e.preventDefault();
...
function bulk_delete() {
  if ($("#jurusanmatkul tbody tr .check:checked").length == 0) {
    ...
    $("#bulk").attr("action", base_url + "department-course/delete");
    Swal({
      title: "You sure?",
